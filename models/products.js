...
NEWSCHEMA('Products').make(function(schema) {
    ...
    schema.setGet(function(error, model, options, callback) {
        ...
        sql.query('lib_suspended_products_count',"select count(*) as total_suspended from products where p_status = 'lib_susp'");
        sql.exec(function(err,response){
         // console.log(response.last10products);
          ...
          console.log(response.lib_suspended_products_count);
        });
        sql.on('end',callback);
    });
});
